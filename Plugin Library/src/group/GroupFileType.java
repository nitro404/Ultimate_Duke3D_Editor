...
public class GroupFileType {
	
	protected String m_name;
	protected Vector<String> m_extensions;
	
	public GroupFileType(String name, String extension) {
		setName(name);
		m_extensions = new Vector<String>();
		addExtension(extension);
	}
	
	public GroupFileType(String name, Vector<String> extensions) {
		setName(name);
		m_extensions = new Vector<String>();
		addExtensions(extensions);
	}
	
	public GroupFileType(String name, String[] extensions) {
		setName(name);
		m_extensions = new Vector<String>();
		addExtensions(extensions);
	}
	...
	public static boolean isValid(GroupFileType fileType) {
		return fileType != null &&
			   fileType.isValid();
	}
	
	public boolean equals(Object o) {
		if(o == null || !(o instanceof GroupFileType)) { return false; }
		GroupFileType t = (GroupFileType) o;
	...
	public String toString() {
		String groupFileType = m_name;
		
		if(m_extensions.size() > 0) {
			groupFileType += " [";
			
			for(int i=0;i<m_extensions.size();i++) {
				groupFileType += m_extensions.elementAt(i);
