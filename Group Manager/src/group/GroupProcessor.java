...
public abstract class GroupProcessor {
	...
	public boolean processGroup(File file) {
		...
		if(group == null) {
			SystemConsole.instance.writeLine("Failed to instantiate \"" + plugin.getName() + " for group processing (" + plugin.getSupportedGroupFileFormatsAsString() + ")\" plugin when attempting to read group file: \"" + file.getName() + "\".");
		...
		try {
			if(!group.load()) {
				SystemConsole.instance.writeLine("Failed to load group for processing: \"" + file.getName() + "\" using plugin: \"" + plugin.getName() + " (" + plugin.getSupportedGroupFileFormatsAsString() + ")\".");
				
				return false;
			}
		}
		catch(HeadlessException e) {
			SystemConsole.instance.writeLine("Exception thrown while loading group for processing: \"" + file.getName() + "\" using plugin: \"" + plugin.getName() + " (" + plugin.getSupportedGroupFileFormatsAsString() + "): " + e.getMessage());
